...
<p><span>{{site.author}}</span><br><span>{{site.street_address}}</span><br><span >{{site.zip_code}} {{site.city}}</span><br><span >{{site.country}}</span></p>
<p><span>Telephone: {{ site.phone }}</span><br><span>Email: {{ site.email }}</span></p>
